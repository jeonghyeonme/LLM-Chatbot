...
- [ ] **[D-4] 취업 정보 크롤링 및 보안 설계**
    - **대상**: 학과 홈페이지 취업 공지사항 (로그인 필요 구역)
    - **핵심**: 관리자 계정을 이용한 서버측 중앙 집중식 크롤링 아키텍처 설계
    - **보안**: 사용자 계정 정보 수집 배제 및 `.env` 기반 보안 관리
...
- [ ] **[U-6] 학과별 취업 정보 (Career) - 추후 예정**
    - **핵심**: 학과 홈페이지 로그인 기반 취업처 및 채용 공고 전용 인터페이스 구축
    - **참고**: 학과별 로그인 세션 연동이 필요한 구역으로 별도 구현 예정
- [ ] **[U-7] 캠퍼스 맵 (`/map`)**
    - **핵심**: 건물 위치 및 시설 정보를 확인할 수 있는 인터랙티브 맵
    - **기술**: SVG 기반 캠퍼스 맵 렌더링 및 [D-3] 시설 데이터 연동
    - **상태**: 현재 홀드스페이스(자리표시자) 상태를 실제 기능으로 구현 예정
