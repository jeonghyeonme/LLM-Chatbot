...
const CareerPage: React.FC = () => {
  ...
  // 데이터 로드: 검색어가 있으면 전체 로드, 없으면 탭 필터링 로드
  useEffect(() => {
    const loadCareers = async () => {
      ...
      try {
        ...
        // 검색어가 없을 때만 탭 필터링 적용 (검색어 있으면 전체 데이터 대상으로 필터링하기 위함)
        if (!searchQuery && activeDept !== '전체') {
          query = query.eq('category', activeDept);
        }

        const { data, error } = await query.limit(searchQuery ? 300 : 100);
        if (error) throw error;
        setCareers(data || []);
      } catch (error) { console.error(error); } finally { setLoading(false); }
    };
    loadCareers();
  }, [activeDept, searchQuery === '']); // 검색어를 다 지웠을 때만 재로딩

  // 클라이언트 사이드 통합 검색 필터 (학과명 + 제목)
  const filteredCareers = careers.filter(career => {
    const cleanQuery = searchQuery.replace(/\s+/g, '').toLowerCase();
    if (!cleanQuery) return true; // 이미 탭 필터링이 적용된 상태

    const combinedText = `${career.category} ${career.title}`.replace(/\s+/g, '').toLowerCase();
      ...
      <main className="flex-1 pt-24 pb-12 px-6 md:px-12 flex flex-col items-center">
        <div className="w-full max-w-6xl">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
            <div>
              <h1 className="text-2xl font-bold text-gray-900 flex items-center gap-2"><Briefcase className="text-inha-blue" />취업 정보 센터</h1>
            ...
            <div className="flex flex-col md:flex-row items-center gap-4 w-full md:w-auto">
              <div className="relative w-full md:w-80">
                ...
                <input 
                  type="text" 
                  placeholder="학과명(전자, 컴퓨터) 또는 공고 제목 검색..."
          ...
          {!searchQuery && (
            <div className="relative group mb-8">
              {showLeftArrow && (
                <button onClick={() => scroll('left')} className="absolute left-0 top-1/2 -translate-y-1/2 z-10 w-8 h-8 flex items-center justify-center bg-white/90 backdrop-blur-sm border border-inha-border rounded-full shadow-sm text-inha-blue"><ChevronLeft size={18} /></button>
              )}
              <div ref={scrollRef} onScroll={checkScroll} className="flex gap-2 overflow-x-auto pb-1 no-scrollbar scroll-smooth">
                {departments.map(dept => (
                  <button key={dept} onClick={() => setActiveDept(dept)} className={`px-5 py-2.5 rounded-full text-sm font-medium whitespace-nowrap transition-all shadow-sm ${activeDept === dept ? 'bg-inha-blue text-white shadow-md' : 'bg-white text-gray-600 border border-inha-border hover:border-inha-blue/30'}`}>{dept}</button>
                ))}
              </div>
              {showRightArrow && (
                <button onClick={() => scroll('right')} className="absolute right-0 top-1/2 -translate-y-1/2 z-10 w-8 h-8 flex items-center justify-center bg-white/90 backdrop-blur-sm border border-inha-border rounded-full shadow-sm text-inha-blue"><ChevronRight size={18} /></button>
              )}
            </div>
          )}
          ...
          <div className="space-y-6">
            {loading ? (
              <div className="flex flex-col items-center justify-center py-20 text-gray-400 gap-4"><Loader2 className="w-10 h-10 animate-spin text-inha-blue" /><p className="font-medium">데이터 로드 중...</p></div>
                  ...
                  <div key={job.id} className="bg-white rounded-inha-card border border-inha-border p-6 hover:shadow-lg hover:border-inha-blue/30 transition-all group flex flex-col justify-between">
                    <div>
                      <div className="flex justify-between items-start mb-4">
                        <div className="p-3 bg-gray-50 rounded-2xl border border-gray-100 group-hover:bg-inha-blue/5 group-hover:border-inha-blue/10 transition-colors"><Building className="w-6 h-6 text-gray-400 group-hover:text-inha-blue" /></div>
                    ...
                    <div className="pt-4 border-t border-gray-50">
                      <div className="flex items-center justify-between text-xs text-gray-500 mb-4"><div className="flex items-center gap-1"><Calendar className="w-3 h-3" />게시일: {job.date}</div></div>
