...
- [x] **[D-2] 공지사항 전면 고도화 (Full Content Scraping)**
    - **변경점**: 단순 목록 수집에서 **상세 본문 전체 수집**으로 전환 (RAG 성능 극대화)
    - **기술**: Playwright 기반 상세 페이지 접근 및 Markdown 변환 저장
    - **저장**: Supabase pgvector에 본문 텍스트 임베딩 적재
- [x] **[D-3] 취업 정보(Career) 전용 크롤러 구축**
    - **대상**: 학과 홈페이지 취업 공지사항 (로그인 필요 구역)
    - **핵심**: 관리자 계정 기반 **Auth-Crawling & Content Caching** 전략 적용
    - **기능**: 부가적인 AI 추출 없이 상세 본문(Markdown) 수집
- [x] **[D-4] 크롤러 파편화 및 병렬화 (Github Actions)**
    - **핵심**: 모놀리식 크롤러를 기능별(Meal, Schedule, Notice, Career)로 분리
    - **자동화**: 개별 워크플로우를 통한 독립적/병렬적 GitHub Actions 실행 구조 구축
    - **장점**: 실행 시간 단축 및 특정 크롤러 장애 시 격리 가능
- [x] **[D-5] 시설 데이터 정적 구축**
    - **대상**: 캠퍼스 내 주요 건물 및 편의시설 위치 정보
    - **기술**: Playwright를 통한 캠퍼스 맵 마커 좌표(위도/경도) 자동 수집
    - **저장**: Supabase `facilities` 테이블 적재 완료
- [x] **[D-6] 온디맨드 멀티모달 처리**
    - **핵심**: 텍스트가 없는 이미지 기반 공지를 해석하기 위해 `analyze_notice_image` 도구 구축
    - **기술**: Gemini 1.5 Flash를 이용한 실시간 Function Calling

### 2. 챗봇 지능 및 백엔드 고도화 (AI/API)
- [x] **[A-1] API 엔드포인트 설계 및 AI 통합**
...
- [ ] **[U-2] 공지사항 정보 센터**
    - **핵심**: 일반공지 및 학과 공지 목록 (`/notices`) 및 상세 페이지 구현 (학사, 행사, 장학, 일반, 메인 채용 공고 포함)
    - **기술**: 리스트-디테일 패턴 UI, 검색 및 카테고리 필터링
- [x] **[U-3] 퀵 버튼 및 추천 질문**
...
- [x] **[U-4] 채팅 고도화**
    ...
    - **기술**: 커스텀 마크다운 파서(Bold, Link 대응) + useRef (자동 스크롤)
...
- [x] **[U-6] 학과별 취업 정보 (`/careers`)**
    - **핵심**: 전 학과 취업처 및 채용 공고 전용 인터페이스 구축
    - **기술**: Supabase 직접 호출(Direct Query)을 통한 실시간 데이터 페칭 및 동적 필터 구성
    - **구조 개선**: 중간 백엔드 API를 생략하고 프론트엔드에서 직접 DB에 접근하여 지연시간(Latency) 최소화
- [x] **[U-7] 캠퍼스 맵 (`/map`)**
    - **핵심**: 건물 위치 및 시설 정보를 확인할 수 있는 인터랙티브 맵
    - **기술**: React Naver Maps API 및 Supabase `facilities` 테이블 연동
    - **상태**: 시설 선택 및 길찾기(transit) 기능 연동 완료
...
- [x] 기본 헬스체크 API 구현
- [x] Playwright 기반 식단/일정/시설/공지사항 크롤러 구현 완료
- [x] 챗봇 도구(Tool) 통합: 식단, 일정, 위치 정보 조회 기능 연동 완료
...
- **2026-05-31**: 캠퍼스 시설 데이터 크롤링 자동화 및 챗봇 위치 정보(네이버 지도) 연동 완료. 프론트엔드 마크다운 링크 렌더링 수정.
