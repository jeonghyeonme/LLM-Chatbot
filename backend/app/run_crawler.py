...
def main():
    ...
    if settings.SUPABASE_URL and settings.SUPABASE_KEY:
        ...
        meal_res = supabase_service.upsert_meals(meals)
        if meal_res:
            print("✅ 식단 데이터 적재 완료")
            
        sch_res = supabase_service.upsert_schedules(schedules)
        if sch_res:
            print("✅ 학사일정 데이터 적재 완료")
    else:
        print("⚠️ Supabase 설정(URL/KEY)이 없어 DB 적재를 건너뜁니다. (.env 파일을 확인해주세요)")
